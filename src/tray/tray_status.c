...
void wt_tray_status_refresh(WT_TrayStatusSummary *summary)
{
    ...
    if (!wt_tray_read_file_utf8(path, &body, &len)) {
        StringCchCopyW(summary->source_label, ARRAYSIZE(summary->source_label),
                       L"Could not read cached scan");
        return;
    }

    summary->available = 1;
    StringCchCopyW(summary->source_path, ARRAYSIZE(summary->source_path), path);
    StringCchCopyW(summary->source_label, ARRAYSIZE(summary->source_label),
    ...
    wchar_t host[64];
    if (wt_tray_json_extract_wstring(body, "hostname", host, ARRAYSIZE(host))) {
        StringCchCopyW(summary->hostname, ARRAYSIZE(summary->hostname), host);
    }

    wchar_t os_name[96];
    if (wt_tray_json_extract_wstring(body, "os", os_name, ARRAYSIZE(os_name))) {
        wchar_t arch[32];
        if (wt_tray_json_extract_wstring(body, "arch", arch, ARRAYSIZE(arch))) {
            StringCchPrintfW(summary->os_line, ARRAYSIZE(summary->os_line),
                             L"%s %s", os_name, arch);
        } else {
            StringCchCopyW(summary->os_line, ARRAYSIZE(summary->os_line), os_name);
        }
    }
                                       ...
                                       wchar_t *out, size_t out_count)
{
    ...
    StringCchPrintfW(
        ...
        L"OS: %s\r\n\r\n"
        L"Source: %s\r\n"
        L"Run Doctor from the tray menu to refresh diagnostics.\r\n"
        L"Mutating actions always require confirmation in the CLI.",
        svc, cpu_line, mem_line,
        s->hostname[0] ? s->hostname : L"(unknown)",
        s->os_line[0] ? s->os_line : L"(unknown)",
        s->source_label);
}
...
HWND wt_tray_status_show(HWND owner)
{
    ...
    HWND hwnd = CreateWindowExW(
        ...
        400, 320, owner, NULL, GetModuleHandleW(NULL), NULL);
    if (hwnd == NULL) {
        return NULL;
    }
